...
#if defined(NDEBUG)
#error "SeedBlockChain cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 50 * COIN;
    }   else if (nHeight < 115001 ) {
        nSubsidy = 40 * COIN;
    }   else if (nHeight < 125001 ) {
        nSubsidy = 30 * COIN;
    }   else if (nHeight < 5235001 ) {
        nSubsidy = 20 * COIN;
    }   else {
        nSubsidy = 20 * COIN;
    }
...
{
    RenameThread("seedblockchain-scriptch");
