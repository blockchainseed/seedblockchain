...
- [Building SeedBlockChain](#building-seedblockchain)
...
Clone the git repositories for seedblockchain and gitian and then checkout the seedblockchain version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/seedblockchain-crypto/seedblockchain.git
cd seedblockchain
...
Follow the instructions in [doc/release-process.md](release-process.md) in the seedblockchain repository
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/seedblockchain/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/seedblockchain-crypto/seedblockchain
...
For example:
```bash
URL=https://github.com/crowning-/seedblockchain.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit seedblockchain=${COMMIT} --url seedblockchain=${URL} ../SeedBlockChain/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit seedblockchain=${COMMIT} --url seedblockchain=${URL} ../SeedBlockChain/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit seedblockchain=${COMMIT} --url seedblockchain=${URL} ../SeedBlockChain/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/seedblockchain-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/seedblockchain-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/seedblockchain-build.assert
```
